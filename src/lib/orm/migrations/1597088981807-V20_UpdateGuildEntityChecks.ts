...
export class V20UpdateGuildEntityChecks1597088981807 implements MigrationInterface {
	...
	private async createNewCheckContraints(queryRunner: QueryRunner): Promise<void> {
		await queryRunner.createCheckConstraints('guilds', [
			...
			new TableCheck({
				name: this.generateCheckName(/* sql */ `("selfmod.attachmentMaximum" >= 0) AND ("selfmod.attachmentMaximum" <= 60)`),
				columnNames: ['"selfmod.attachmentMaximum"'],
				expression: /* sql */ `("selfmod.attachmentMaximum" >= 0) AND ("selfmod.attachmentMaximum" <= 60)`
			}),
			new TableCheck({
				name: this.generateCheckName(/* sql */ `("selfmod.attachmentDuration" >= 5000) AND ("selfmod.attachmentDuration" <= 120000)`),
				columnNames: ['"selfmod.attachmentDuration"'],
				expression: /* sql */ `("selfmod.attachmentDuration" >= 5000) AND ("selfmod.attachmentDuration" <= 120000)`
			}),
			new TableCheck({
				name: this.generateCheckName(/* sql */ `"selfmod.attachmentPunishmentDuration" >= 1000`),
				columnNames: ['"selfmod.attachmentPunishmentDuration"'],
				expression: /* sql */ `"selfmod.attachmentPunishmentDuration" >= 1000`
			}),
			new TableCheck({
